(function (root, factory) {
    ...
    function (root) {
            ...
            for (
                ...
                if (firstChar === '[') {
                    if (elem.hasAttribute(attribute[0])) {
                        if (value) {
                            if (
                                elem.getAttribute(attribute[0]) === attribute[1]
                            ) {
                                return elem;
                            }
                        } else {
         ...
         * @return {Number}
         */
        const easingPattern = function (type, time) {
            let pattern;
         ...
         * @return {Number}
         */
        const getEndLocation = function (anchor, headerHeight, offset) {
            let location = 0;
        ...
        smoothScroll.animateScroll = function (anchor, toggle, options) {
            ...
            const overrides = getDataOptions(
                toggle ? toggle.getAttribute('data-options') : null
            );
            const animateSettings = extend(
                settings || defaults,
                options || {},
                overrides
            ...
            const startLocation = root.pageYOffset; // Current location on the page
            if (animateSettings.selectorHeader && !fixedHeader) {
                // Get the fixed header if not already set
                fixedHeader = document.querySelector(
             ...
             */
            const stopAnimateScroll = function (
                position,
                endLocation,
                animationInterval
            ) {
                const currentLocation = root.pageYOffset;
                if (
             ...
             * @private
             */
            const loopAnimateScroll = function () {
                timeLapsed += 16;
                ...
                position =
                    startLocation +
                    distance *
                        easingPattern(animateSettings.easing, percentage);
             ...
             * @private
             */
            const startAnimateScroll = function () {
                clearInterval(animationInterval);
         ...
         * @private
         */
        const hashChangeHandler = function (event) {
            // Get hash from URL
        ...
        const clickHandler = function (event) {
            ...
            const hash = escapeCharacters(toggle.hash);
            ...
            if (hash === '#') {
                ...
                const id = anchor.id ? anchor.id : 'smooth-scroll-top';
