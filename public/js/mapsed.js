...
(function () {
    ...
    const _version = '(0.4)';
    const _plugInName = 'mapsed';
    let _plugInInstances = 1;
    $.fn.mapsed = function (options) {
        // consts
        // - Centre of the UK (ish ...)
        const DEFAULT_CENTER = new google.maps.LatLng(53.175148, -1.423908);
        const DEFAULT_ZOOM = 10;

        // private plug-in variables
        const _plugIn = this; // Reference back to the "mapsed" plug-in instance
        let _searchBox = null; // Search box that appears on the map
        let _gmSearchBox = null; // Google (autocompleting) Search box the underlying input text box is twinned with
        let _searchBtn = null; // Button to click to confirm search should be applied (not strictly needed (ENTER does the same), but users may be confused if there isn't one!)
        let _moreBtn = null; // Available when more results are available for a result set (Google Places API pages the results)
        let _pageNum = 0; // Keeps track of how many pages of results are shown (used to reset the markers on a new search)
        let _gMap = null; // Underlying Google maps object for the div
        var _mapContainer = null; // jQuery reference to the DIV the map is in
        let _placesApi = null; // Reference to the Google Places API object
        let _markers = []; // Set of markers displayed on the map
        let _instance = -1; // Instance "this" plug-in is managing (so we can support zmultiple maps on the page)
        let _fullWin = false; // Flags "mapsed" is in full-window mode, which means "mapsed" created the DIV we're in
        let _firstSearch = true; // Used to ensure we don't clear markers when the map is drawn for the first time (so any "showOnLoad" markers aren't cleared)
        let _hasMapInitFired = false; // Used to flag initialisation of the map (after Google Maps API has finished drawing it)
        let _areBoundsSet = false; // Used to flag that an event has set the boundary (so we don't set the zoom/center manually as GM will calc this for us)
        let _helpBtn = null; // Reference to the help dialog button ([?])
        let _helpDlg = null; // Reference to the help dialog that is toggled by the help button
        let _closeBtn = null; // Reference to the close button (only used in full-window mode)
        let _addBtn = null; // Reference to the add button ([+])
        let _geoBtn = null; // Reference to the Geo location button [(*)]
        let gm = null; // Short cut reference to the Google Maps namespace (this is initialised in the constructor to give the Google API time to load on the page)
        let gp = null; // Short cut reference to the Google Places namespace (this is initialised in the constructor to give the Google API time to load on the page)
        // / <summary>
        // / Plug-in options:
        ...
        this.setMapCentreByGeo = function () {
            ...
            navigator.geolocation.getCurrentPosition(
                function (geoPos) {
                    ...
                    const so = settings.searchOptions;
                    if (so && so.geoSearch && so.geoSearch.length > 0) {
                        const newLocation = pos.toUrlValue();
                        const search = so.geoSearch.replace(
                            '{POSITION}',
                            newLocation
                        );
        ...
        function onPlaceSelect(element) {
            const $root = element.parents('.mapsed-root');
            const $vw = element.parents('.mapsed-view');
            const model = getViewModel($vw);
        ...
        function onPlaceEdit(element) {
            const $root = element.parents('.mapsed-root');
            const lat = $root.find('.mapsed-lat').val();
            const lng = $root.find('.mapsed-lng').val();
        ...
        function onPlaceAdd(evt) {
            ...
            gm.event.addListener(newMarker, 'click', function (evt) {
                const currMarker = this;
                closeTooltips();

                if (settings.onAdd) {
                    const root = $(currMarker.tooltip.content);
            ...
            gm.event.addListener(newMarker, 'dragend', function (evt) {
                ...
                const tip = $(currMarker.tooltip.content);
        ...
        function onPlaceDelete(element) {
            const $root = element.parents('.mapsed-root');
            const $vw = $root.find('.mapsed-view');
            const model = getViewModel($vw);

            if (settings.onDelete(_plugIn, model)) {
                // find the appropriate marker
                const marker = findMarker(model.lat, model.lng);
        ...
        function onPlaceSave(element) {
            ...
            const marker = findMarker(place.lat, place.lng);
        ...
        function gmMapLoaded() {
            ...
            const gmToolBtns = $(_mapContainer.find('.gm-style-mtc'));
            if (gmToolBtns.length > 0) {
                const gmLineHeight = $(gmToolBtns[0]).css('line-height');
        ...
        function gmPlaceSelected(places, status, pagination) {
            ...
            for (var i = 0, place; (place = places[i]); i++) {
                ...
                const marker = addMarker(place, pos, 'google', bounds);
        ...
        function buildMsg(title, msg, doConfirm, prompt, callback) {
            ...
            html =
                ...
                `<h3>${title}</h3>` +
                '<div>' +
                `<div class='mapsed-modal-message'>${msg}</div>${buttons}</div>` +
                '</div>';
        ...
        function applyTemplate(tmpl, model, $ctx) {
            ...
            if (model.photo) {
                const path = model.photo.getUrl({ maxWidth: '70' });
        ...
        function hideEmpty(model, $vw) {
            ...
            const $telNo = $vw.find('.mapsed-telNo');
            const $ws = $vw.find('.mapsed-website');
            const $url = $vw.find('.mapsed-url');
            ...
            const settings = _plugIn.getSettings();
        ...
        // / </summary>
        function addSearch() {
            const id = `mapsed-search-box-${_instance}`;
            ...
            const so = settings.searchOptions;
            let html = `<input type='text' id='${id}' class='mapsed-searchbox' autocomplete='off' `;
            html += "placeholder='";
            ...
            gm.event.addListener(_gmSearchBox, 'places_changed', function () {
                const searchFor = _searchBox.val();
                doSearch(searchFor);
            });
        ...
        function addCloseButton() {
            ...
            const onCloseEvent = function (evt) {
                evt.preventDefault();
        ...
        function addGeoLocationButton() {
            ...
            const onClickEvent = function (evt) {
                _plugIn.setMapCentreByGeo();
            };
        ...
        function addHelpButton() {
            ...
            const helpHtml = settings.getHelpWindow();
            _helpDlg = $(helpHtml)
        ...
        function createControlButton(
            ...
            if (buttonText && buttonText.length > 0) {
                const arrSplit = buttonText.split('|');
                buttonText = arrSplit[0];
                tooltip = arrSplit.length > 1 ? ` title='${arrSplit[1]}'` : '';
            }

            var markUp = `<button ${classes}${tooltip}>${buttonText}</button>`;
        ...
        function attachTooltip(forMarker) {
            ...
            const newId = _markers.length + 1;
            const d = forMarker.details;
            forMarker.id = newId;
            ...
            var item = $(
                `<div id='mapsed-${newId}' class='mapsed-root'>` +
                    ...
                    }' />${getViewTemplate()}${getEditTemplate()}</div>`
            );
        ...
        function showTooltip(inRwMode) {
            ...
            const model = marker.details;
            const $ele = $(tip.getContent());
            const $ro = $ele.find('.mapsed-view');
            const $rw = $ele.find('.mapsed-edit');
            const settings = _plugIn.getSettings();
        ...
        function getViewModel($vw) {
            const $root = $vw.parents('.mapsed-root');
            const model = {
        ...
        function doSearch(searchFor) {
            const boundary = _gMap.getBounds();
            let location = null;
            const re = /(\-?\d+(\.\d+)?),\s*(\-?\d+(\.\d+)?)/gi; // regex for lat/lng co-ords
            let query = searchFor;
            ...
            const latLngMatches = query.match(re);
        ...
        function normalisePlacesApiAddress(details, fromGP) {
            const ac = fromGP.address_components;

            // Copy Googles version of an address to something more useable for us
            let street = findPart(ac, 'street_address');
            if (street === '') {
                // not present so fallback to "route"
                street = findPart(ac, 'route');
            }

            const town = findPart(ac, 'locality');
            const area = findPart(ac, 'administrative_area_level_1');
            const postCode = findPart(ac, 'postal_code');
            const country = findPart(ac, 'country');
        ...
        function findPart(addressParts, typeName, getShortVersion) {
            ...
            for (let i = 0; i < addressParts.length; i++) {
                const item = addressParts[i];
                let found = false;
                for (let j = 0; j < item.types.length; j++) {
                    const addrType = item.types[j];
        ...
        const ctor = function () {
                ...
                _mapContainer.on(
                    ...
                    function () {
                        ...
                        if (settings.confirmDelete) {
                            const $vw = element.parents('.mapsed-view');
                            const model = getViewModel($vw);
                            const msg = `<strong>${model.name}</strong> will be deleted.`;
                            _plugIn.confirmMsg(
                                'Confirm Delete',
                                msg,
                                'Are you sure?',
            ...
            const plugger = `plugin_${_plugInName}`;
        ...
        if (!this.length) {
            const mapId = 'mapsed-full-window';
            var _mapContainer = $(`#${mapId}`); // see if we've already added one
